    ...
    for src, tgt, src_len, tgt_len, original_src, original_tgt in trainloader:
        ...
        try:
            if config.schesamp:
                if epoch > 8:
                    e = epoch - 8
                    loss, outputs = model(src, lengths, dec, targets, tgt_len, teacher_ratio=0.9**e)

                else:
                    loss, outputs = model(src, lengths, dec, targets, tgt_len)
            else:
                loss, outputs = model(src, lengths, dec, targets, tgt_len)
        ...
        if params['updates'] % config.eval_interval == 0:
            ...
            if opt.tensorboard:
                writer.add_scalar("train" + "/loss", params['report_loss'], params['updates'])
                writer.add_scalar("train" + "/lr", optim.lr, params['updates'])
                writer.add_scalar("train" + "/accuracy", params['report_correct'] * 100.0 / params['report_total'], params['updates'])
            ...
            for metric in config.metrics:
                ...
                if score[metric] >= max(params[metric]):
                    ...
                    if opt.tensorboard:
                        writer.add_scalar("valid" + "/" + metric, score[metric], params['updates'])

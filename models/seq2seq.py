...
class seq2seq(nn.Module):
    ...
    def __init__(self, config, use_attention=True, encoder=None, decoder=None):
        ...
        if decoder is not None:
            ...
            self.decoder = models.rnn_decoder(config, embedding=tgt_embedding, use_attention=use_attention, score_fn=self.score_fn)
    ...
    def forward(self, src, src_len, dec, targets, target_length=None, teacher_ratio=1.0):
        src = src.t()
        dec = dec.t()
        targets = targets.t()
        teacher = random.random() < teacher_ratio

        contexts, state_src = self.encoder(src, src_len.data.tolist())
        if self.config.sae:
            tgt_len = target_length - 2
            target_lengths, indices = torch.sort(tgt_len, dim=0, descending=True)
            contexts_ae, state_tgt = self.encoder_ae(targets[1:-1], target_lengths.data.tolist())
        ...
        if teacher:
            for input in dec.split(1):
                output, state, attn_weights = self.decoder(input.squeeze(0), state_src)
                outputs.append(output)
            outputs = torch.stack(outputs)
            if self.config.sae:
                for input in dec.split(1):
                    output_ae, state_ae, _ = self.decoder(input.squeeze(0), state_tgt, attention=False)
                    outputs_ae.append(output_ae)
                outputs_ae = torch.stack(outputs_ae)
        else:
        ...
        if self.config.sae:
            ...
            if self.config.ae_weight > 0:
                ...
                loss += self.config.ae_weight * loss_reg
